...
public class AssumeTest implements LogicNGTest {
    ...
    public void testAssume() throws ParserException {
        ...
        for (final SatSolver s : solvers) {
            ...
            assertThat(s.satCall().addFormula(f.literal("a", false)).sat().getResult()).isTrue();
            assertThat(s.satCall().addFormula(f.variable("b")).sat().getResult()).isTrue();
            assertThat(s.satCall().addFormula(f.variable("c")).sat().getResult()).isTrue();
            assertThat(s.satCall().addFormula(f.variable("d")).sat().getResult()).isTrue();
            assertThat(s.satCall().addFormula(f.variable("e")).sat().getResult()).isTrue();
            assertThat(s.satCall().addFormula(f.variable("f")).sat().getResult()).isTrue();
            assertThat(s.satCall().addFormula(f.variable("g")).sat().getResult()).isTrue();
            assertThat(s.satCall().addFormula(f.variable("a")).sat().getResult()).isFalse();
            assertThat(s.satCall().addFormula(f.literal("b", false)).sat().getResult()).isFalse();
            assertThat(s.satCall().addFormula(f.literal("c", false)).sat().getResult()).isFalse();
            assertThat(s.satCall().addFormula(f.literal("d", false)).sat().getResult()).isFalse();
            assertThat(s.satCall().addFormula(f.literal("e", false)).sat().getResult()).isFalse();
            assertThat(s.satCall().addFormula(f.literal("f", false)).sat().getResult()).isFalse();
            assertThat(s.satCall().addFormula(f.literal("g", false)).sat().getResult()).isTrue();
