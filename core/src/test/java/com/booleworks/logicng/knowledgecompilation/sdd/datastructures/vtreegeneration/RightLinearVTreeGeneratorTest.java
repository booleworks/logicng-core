...
public class RightLinearVTreeGeneratorTest {
    @Test
    public void testSimpleCases() {
        ...
        assertThat(isRightLinear(v1.getRoot())).isTrue();
        ...
        assertThat(isRightLinear(v2.getRoot())).isTrue();
        ...
        assertThat(isRightLinear(v3.getRoot())).isTrue();
        ...
        assertThat(isRightLinear(v5.getRoot())).isTrue();
    }
    ...
    public void testWithFormula() throws IOException {
        ...
        assertThat(isRightLinear(root.getRoot())).isTrue();
    }


    private VTreeRoot generateFor(final Set<Variable> vars) {
        final VTreeRoot.Builder builder = VTreeRoot.builder();
        final VTree vtree = new RightLinearVTreeGenerator(vars).generate(builder);
        return builder.build(vtree);
    }

    private boolean isRightLinear(final VTree vtree) {
        if (vtree.isLeaf()) {
            return true;
        } else {
            final VTreeInternal node = (VTreeInternal) vtree;
            return node.getLeft().isLeaf() && isRightLinear(node.getRight());
        }
    }
}
