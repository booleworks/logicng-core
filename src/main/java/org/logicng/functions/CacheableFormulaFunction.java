...
/**
 * A cacheable formula function does not hold an internal mutable state but does use
 * an internal cache to speed up computations.  This cache is usually changed by the
 * function.  Formulas from a caching formula factory provide their cache via the factory,
 * formulas from a non-caching formula factory can be given their own cache per function.
 * A cacheable formula function with a provided cache is not thread-safe.
 * @version 3.0.0
 * @since 3.0.0
 */
public abstract class CacheableFormulaFunction<T> implements FormulaFunction<T> {
    ...
    /**
     * Creates a new cacheable formula function.  For a caching formula factory, the cache for
    ...
    protected CacheableFormulaFunction(final FormulaFactory f, final CacheEntry cacheEntry) {
        this(f, f instanceof CachingFormulaFactory ? ((CachingFormulaFactory) f).getFunctionCacheForType(cacheEntry) : null);
    }

    /**
     * Creates a new cacheable formula function with a given cache.  This cache will always be used
