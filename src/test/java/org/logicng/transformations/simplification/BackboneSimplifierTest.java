...
public class BackboneSimplifierTest extends TestWithFormulaContext {
    ...
    public void testTrivialBackbones(final FormulaContext _c) throws ParserException {
        ...
        assertThat(_c.p.parse("A & (A => B) & ~B").transform(backboneSimplifier)).isEqualTo(_c.p.parse("$false"));
    ...
    public void testRealBackbones(final FormulaContext _c) throws ParserException {
        ...
        assertThat(_c.p.parse("A & B & (B | C)").transform(backboneSimplifier)).isEqualTo(_c.p.parse("A & B"));
        assertThat(_c.p.parse("A & B & (~B | C)").transform(backboneSimplifier)).isEqualTo(_c.p.parse("A & B & C"));
        assertThat(_c.p.parse("A & B & (~B | C) & (B | D) & (A => F)").transform(backboneSimplifier)).isEqualTo(_c.p.parse("A & B & C & F"));
        assertThat(_c.p.parse("X & Y & (~B | C) & (B | D) & (A => F)").transform(backboneSimplifier)).isEqualTo(_c.p.parse("X & Y & (~B | C) & (B | D) & (A => F)"));
        assertThat(_c.p.parse("~A & ~B & (~B | C) & (B | D) & (A => F)").transform(backboneSimplifier)).isEqualTo(_c.p.parse("~A & ~B & D"));
    }
}
