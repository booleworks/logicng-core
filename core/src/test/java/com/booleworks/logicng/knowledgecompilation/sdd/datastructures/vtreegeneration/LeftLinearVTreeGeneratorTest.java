...
public class LeftLinearVTreeGeneratorTest {
    @Test
    public void testSimpleCases() {
        ...
        assertThat(isLeftLinear(v1.getRoot())).isTrue();
        ...
        assertThat(isLeftLinear(v2.getRoot())).isTrue();
        ...
        assertThat(isLeftLinear(v3.getRoot())).isTrue();
        ...
        assertThat(isLeftLinear(v5.getRoot())).isTrue();
    }
    ...
    public void testWithFormula() throws IOException {
        ...
        assertThat(isLeftLinear(root.getRoot())).isTrue();
    }

    private VTreeRoot generateFor(final Set<Variable> vars) {
        final VTreeRoot.Builder builder = VTreeRoot.builder();
        final VTree vtree = new LeftLinearVTreeGenerator(vars).generate(builder);
        return builder.build(vtree);
    }

    private boolean isLeftLinear(final VTree vtree) {
        if (vtree.isLeaf()) {
            return true;
        } else {
            final VTreeInternal node = (VTreeInternal) vtree;
            return node.getRight().isLeaf() && isLeftLinear(node.getLeft());
        }
    }
}
