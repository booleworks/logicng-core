...
/**
 * A split variable provider which provides split variables which occur
 * particularly seldom in the formulas on the solver. The variables occurring in
 * the formulas are sorted by their occurrence. This provider returns those
 * variables with the smallest occurrence.
 * @version 3.0.0
 * @since 3.0.0
 */
public class LeastCommonVariablesProvider extends SplitVariableProviderWithTakeRate {
     ...
     * Creates a split variable provider returning the least common variables.
     * <p>
     * The take rate specifies the number of variables which should be returned
     * in {@link #getSplitVars}. So the result will contain
     * {@code Math.min(maximumNumberOfVariables, (int) Math.ceil(variables.size() * takeRate))}
     * variables.
