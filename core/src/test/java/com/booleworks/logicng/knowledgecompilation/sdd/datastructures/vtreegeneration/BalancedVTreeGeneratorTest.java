...
public class BalancedVTreeGeneratorTest {
    @Test
    public void testSimpleCases() {
        ...
        assertThat(isBalanced(v1.getRoot())).isTrue();
        ...
        assertThat(isBalanced(v2.getRoot())).isTrue();
        ...
        assertThat(isBalanced(v3.getRoot())).isTrue();
        ...
        assertThat(isBalanced(v5.getRoot())).isTrue();
    }
    ...
    public void testWithFormula() throws IOException {
        ...
        assertThat(isBalanced(root.getRoot())).isTrue();
    }

    private VTreeRoot generateFor(final Set<Variable> vars) {
        final VTreeRoot.Builder builder = VTreeRoot.builder();
        final VTree vtree = new BalancedVTreeGenerator(vars).generate(builder);
        return builder.build(vtree);
    }

    private boolean isBalanced(final VTree vtree) {
        if (vtree.isLeaf()) {
            return true;
        } else {
            final VTreeInternal node = (VTreeInternal) vtree;
            final int leftCount = VTreeUtil.varCount(node.getLeft());
            final int rightCount = VTreeUtil.varCount(node.getRight());
            return Math.abs(leftCount - rightCount) <= 1 && isBalanced(node.getLeft()) && isBalanced(node.getRight());
        }
    }
}
